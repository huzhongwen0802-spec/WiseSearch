...
call scripts\windows\start_opencli_browser.cmd
if errorlevel 1 (
...
call scripts\windows\check_opencli.cmd 2>nul | findstr /L /C:"[OK] Connectivity" >nul
...
echo Log: %~dp0logs\streamlit.log
...
if exist "%~dp0logs\streamlit.log" move /y "%~dp0logs\streamlit.log" "%~dp0logs\streamlit_%LOGSTAMP%.log" >nul
if exist "%~dp0logs\streamlit.error.log" move /y "%~dp0logs\streamlit.error.log" "%~dp0logs\streamlit_error_%LOGSTAMP%.log" >nul

"%PYTHON%" -m streamlit run app.py > "%~dp0logs\streamlit.log" 2>&1

echo.
echo Streamlit stopped. Check logs\streamlit.log for details.
endlocal
